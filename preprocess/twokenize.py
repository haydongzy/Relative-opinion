...
#        print(' '.join(tokenizeRawTweetText(line)))
    tag = set(['#hehe', '#haha'])
    line = "here's the guy #hehe #heihei @rncwomen @womentrump2016 @gop have as a candidate-pervert @realdonaldtrump for president? wakala! b… https://t.co/u8217spont"
    print([tk for tk in tokenizeRawTweetText(line) if not (tk.startswith('@') or re.match(url, tk) or re.match(punctSeq, tk) or (tk.startswith('#') and (tk in tag)))])
